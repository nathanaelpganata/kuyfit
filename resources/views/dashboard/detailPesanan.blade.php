    ...
    <div class="h-screen w-full bg-[#F4F4F4]">
        ...
        <div class="flex flex-row justify-between items-center m-10 mb-16">
            ...
            @if($detail->status == 'pending')
                <div class="space-x-2">
                    <a href="/my/pesanan/detail/{{ $detail->id }}/update/{{ $status = 'ongoing' }}" class="shadow bg-[#27698F] hover:brightness-110 focus:shadow-outline focus:outline-none text-white text-base font-bold tracking-wider py-2 px-4 rounded-lg">Konfirmasi</a>
                    <a href="/my/pesanan/detail/{{ $detail->id }}/update/{{ $status = 'canceled' }}" class="shadow bg-[#445863] hover:brightness-110 focus:shadow-outline focus:outline-none text-white text-base font-bold tracking-wider py-2 px-4 rounded-lg">Tolak</a>
                 </div>
            @elseif($detail->status == 'ongoing')
                <div class="space-x-2">
                    <div class="shadow bg-[#27698F] focus:shadow-outline focus:outline-none text-white text-base font-bold tracking-wider py-2 px-4 rounded-lg">Pesanan Sedang Berjalang</div>
                </div>
             @elseif($detail->status == 'finished')
                <div class="space-x-2">
                    <div class="shadow bg-[#27698F] focus:shadow-outline focus:outline-none text-white text-base font-bold tracking-wider py-2 px-4 rounded-lg">Pesanan Berakhir</div>
                </div>
            @endif
        </div>
            ...
            <div class="flex flex-col w-4/5 space-y-4 bg-[#8ED6FF] rounded-2xl">
                <form class="w-4/5 m-12">
                    ...
                    <div class="flex flex-row items-center mb-6">
                        ...
                        <div class="w-2/3">
                            <input id="timestamp" type="datetime" name="timestamp" class="bg-[#3F95C5] border-4 border-none rounded-lg w-full py-2 px-4 text-white leading-tight hover:brightness-110 focus:bg-white focus:outline-[#3F95C5] focus:text-black disabled:brightness-95" value="{{ $detail->timeStamp }}" disabled>
                        </div>
                    </div>
